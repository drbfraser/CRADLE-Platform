...
export const assessmentFormValidationSchema = Yup.object().shape({
  ...
  .test("atLeastOneRequired", "One of first 4 fields required", function(values) {
    const isValid =
      [AssessmentField.investigation,
      AssessmentField.finalDiagnosis,
      AssessmentField.treatment,
      AssessmentField.drugHistory]
        .some(field => {return !!(values[field])})
    if (isValid) {
      return true;
    } else {
      return this.createError({
        path: AssessmentField.investigation || AssessmentField.finalDiagnosis ||
          AssessmentField.treatment || AssessmentField.drugHistory,
        message: "At least one of Investigation Results, Final Diagnosis, Treatment/Operation, and Drug History must be entered"
      })
    }
  })
