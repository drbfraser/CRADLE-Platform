...
class FormQuestion(QuestionBase, extra="forbid"):
    ...
    def validate_constraints(self) -> Self:
        # Check number limits
        if self.num_min is not None and self.answers and self.answers.number is not None:
            if self.answers.number < self.num_min:
                raise ValueError(f"answer {self.answers.number} below minimum {self.num_min}")

        if self.num_max is not None and self.answers and self.answers.number is not None:
            if self.answers.number > self.num_max:
                raise ValueError(f"answer {self.answers.number} above maximum {self.num_max}")
        ...
        if self.string_max_length is not None and self.answers and self.answers.text:
            if len(self.answers.text) > self.string_max_length:
                raise ValueError(f"answer text exceeds max length {self.string_max_length}")
