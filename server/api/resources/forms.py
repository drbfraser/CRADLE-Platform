...
from common.api_utils import FormIdPath
...
def validate_question_constraints(question):
    """
    Validate constraints for a question.
    - Ensure required fields are not empty.
    - Validate numerical fields against min/max.
    """
    if question.get("required") and not question["answers"]:
        raise ValueError(f"Question '{question['question_text']}' is required but missing.")

    if question["question_type"] == "integer" and "number" in question["answers"]:
        value = question["answers"]["number"]
        min_value = question.get("min_value")
        max_value = question.get("max_value")

        if min_value is not None and value < min_value:
            raise ValueError(
                f"Value for '{question['question_text']}' must be at least {min_value}."
            )
        if max_value is not None and value > max_value:
            raise ValueError(
                f"Value for '{question['question_text']}' must not exceed {max_value}."
            )
...
def submit_form(body: FormModel):
    """Submit Form"""
    if body.id is not None and crud.read(FormOrm, id=body.id):
    ...
    if body.form_template_id is not None:
        ...
        if form_template.form_classification_id != body.form_classification_id:
            return abort(400, description="Form classification does not match Template.")
    ...
    for question in body.questions:
        try:
            validate_question_constraints(question)
