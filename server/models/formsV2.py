...
class LangVersionOrmV2(db.Model):
    ...
    __table_args__ = (
        db.PrimaryKeyConstraint('string_id', 'lang'),
    )
...
class FormQuestionTemplateOrmV2(db.Model):
    ...
    form_template_id = db.Column(
        db.String(50), 
        db.ForeignKey("form_template_v2.id"),
        ondelete="SET NULL",
        index=True
    )
...
class FormSubmissionOrmV2(db.Model):
    ...
    last_edited = db.Column(db.BigInteger, nullable=False, default=get_current_time, onupdate=get_current_time)
