...
def test_create_workflow_classification_success(database, api_post):
    ...
    try:
        ...
        assert response.json()["name"] == "Test Classification"
        ...
        assert created_classification.name_string_id is not None
...
def test_patch_workflow_classification(database, api_get, api_post, api_patch):
    ...
    try:
        ...
        assert response.json()["name"] == "updated_classification_name"
...
def test_get_workflow_classifications_with_data(database, api_get, api_post):
    ...
    try:
        ...
        api_post(endpoint="/api/workflow/classifications",
                 json=classification1_data)
        api_post(endpoint="/api/workflow/classifications",
                 json=classification2_data)
        api_post(endpoint="/api/workflow/classifications",
                 json=classification3_data)
        database.session.commit()
        ...
        classification_names = [item["name"] for item in raw_items]
...
def test_get_single_workflow_classification_success(database, api_get, api_post):
    ...
    classification_data = {
        "id": get_uuid(),
        "name": {"English": "Test Single Classification"},
    }
    ...
    try:
        ...
        assert response.status_code == 200
        assert response_body["id"] == classification_data["id"]
        assert response.json()["name"] == "Test Single Classification"
    finally:
        crud.delete_workflow_classification(
            WorkflowClassificationOrm, id=classification_data["id"]
        )
