...
def test_override_current_step(sequential_workflow_view):
    workflow_view = sequential_workflow_view
    workflow_view.instance.patient_id = "test-patient-123"
    ...
    with patch(
        ...
        # Skip the current active step (revert to pending)
        WorkflowPlanner.apply_action(workflow_view, actions[1])
        check_workflow_instance_state(
            workflow_view.instance,
            status="Active",
            current_step_id="si-1",
