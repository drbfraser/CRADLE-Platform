...
def patient_orm():
    ...
    p.id = "p-1"
    p.name = "Mary Brown"
    p.sex = SexEnum.FEMALE  # will be converted to .value
    p.is_pregnant = True
    p.medical_history = None
    p.drug_history = None
    p.allergy = "Peanuts"
    p.zone = "1"
    p.date_of_birth = dt.date(1998, 1, 1)  # real date object
...
def test_patient_shallow_sets_base_and_converts_types(patient_orm):
    ...
    assert out["date_of_birth"] == "1998-01-01"  # stringified
...
def test_patient_non_shallow_includes_nested_lists(patient_orm):
    # Add one reading to prove it appears
