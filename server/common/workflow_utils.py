...
def assign_branch_id(branch: dict, step_id: str) -> None:
    """
    ...
    if branch["condition"] is not None:
        if branch["condition"]["id"] is None:
            branch["condition"]["id"] = get_uuid()

        branch["condition_id"] = branch["condition"]["id"]


def assign_step_ids(m: Type[M], step: dict, workflow_id: str) -> None:
    """
    ...
    for branch in step["branches"]:
        assign_branch_id(branch, step_id)


def assign_workflow_template_or_instance_ids(m: Type[M], workflow: dict) -> None:
    """
