...
class WorkflowDataResolver:
    ...
    def resolve_data_for_branches(
        ...
        from service.workflow.datasourcing.data_sourcing import resolve_datasources
        
        variables = self.extract_variables_from_branches(branches)
        datasources = [f"${var}" for var in variables]
        resolved_data = resolve_datasources(patient_id, datasources, self.catalogue)
